  ...
  return (
    <AdminLayout title="Corrections">
      ...
      {loading ? (
        ...
        <p className="mt-8 text-slate-400">No {status} reports.</p>
      ) : (
        <ul className="mt-6 space-y-4">
          {corrections.map((c) => (
            <li key={c._id} className="rounded-xl border border-white/10 bg-white/[0.03] p-5">
              <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-slate-400">
                <span className="font-semibold text-brand-300">{c.contentType}</span>
                <time dateTime={c.createdAt}>{new Date(c.createdAt).toLocaleString()}</time>
              </div>
              <p className="mt-2 whitespace-pre-line text-sm text-slate-200">{c.note}</p>
              {c.submitterEmail && <p className="mt-1 text-xs text-slate-500">From: {c.submitterEmail}</p>}

              <div className="mt-4 flex flex-wrap gap-2">
                <Link
                  to={`${EDIT_PATH[c.contentType] || "/admin"}?edit=${c.contentId}`}
                  className="rounded-full border border-brand-400/40 px-4 py-1.5 text-sm text-brand-300 hover:bg-brand-500/10"
                >
                  Edit content
                </Link>
                {status !== "resolved" && (
