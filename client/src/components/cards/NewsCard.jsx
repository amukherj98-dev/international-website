  ...
  return (
    <MotionCard as="a" href={article.url} target="_blank" rel="noreferrer" className="block">
      <div className="flex items-center justify-between gap-2 text-xs text-slate-500">
        <span className="rounded-full bg-slate-900/5 px-2.5 py-1 font-medium text-brand-600">
      ...
      {article.description && <p className="mt-2 text-sm leading-relaxed text-slate-700">{article.description}</p>}
      <p className="mt-3 text-xs text-slate-400">{article.sourceName || "Source"}</p>
    </MotionCard>
  );
}
